...
if [ "$PLATFORM" == "pi" ]; then
    ...
    echo "    uv run python scripts/test_rolling_buffer_persist.py --setup"
    echo "    # Then power cycle the radar (unplug USB, wait 3s, replug)"
    echo "    uv run python scripts/test_rolling_buffer_persist.py --test"
    echo ""
fi
...
if [ "$PLATFORM" == "pi" ]; then
    ...
    echo "    sudo scripts/setup_alloy.sh"
    echo ""
